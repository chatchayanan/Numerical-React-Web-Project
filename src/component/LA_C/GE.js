...
const GE = () => {
  ...
  const GE_M = () => {
      ...
      for (var k = 0; k < MatrixSize; k++) {
       ...
       var X = new Array(MatrixSize);
       var X_ans = new Array(MatrixSize);

       //? หา Xn (X ตัวสุดท้าย)
       X[MatrixSize - 1] = Math.round(
         b[MatrixSize - 1] /
         A[MatrixSize - 1][MatrixSize - 1]
       );
       
       for (i = MatrixSize - 2; i >= 0; i--) {

        //? หา Xn-1 to X1 (หาแบบย้อนกลับ)
    ...
    axios
      ...
      .then((response) => {
        for (let i = 0; i < response.data.result.length; i++) {
          if (response.data.result[i].id === "GaussEliminate") {
  ...
  return (
    <div>
      <h1 className="myheader">Gauss Eliminate Method</h1>
